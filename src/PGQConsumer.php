...
/**
 * pgq\PGQConsumer is a pgq\SystemDaemon providing the pgq\PGQ SQL API for PHP
 ...
 * pgq\PGQRemoteConsumer and pgq\PGQEventRemoteConsumer.
 *
 * Online references about pgq\PGQ are to be found here:
 ...
 *
 * See pgq\SystemDaemon header documentation for details about first two
 * functions.
 *
 * The consumer model provided by pgq\PGQConsumer works by letting the
 ...
 * PGQ_ABORT_BATCH and pgq\PGQConsumer will handle: batch is not finished,
 * the error is considered not fatal.
 *
 * When you return PGQ_EVENT_RETRY or PGQ_EVENT_FAILED, pgq\PGQConsumer
 * will call the matching pgq SQL function to tag the Event. In the
 * retry case, pgq\PGQConsumer will get $event->retry_delay attribute to
 * give to the SQL level API, and in the failed case,
 * $event->failed_reason.
 *
 * pgq\PGQConsumer supports same commands as pgq\SystemDaemon, and some more:
 *   install, uninstall, create_queue, drop_queue, register, unregister
 *   failed, delete <event_id|all>, retry <event_id|all>
 *
 * See pgq\SystemDaemon documentation for using this class at PHP and
 * command line levels.
 */
abstract class PGQConsumer extends SystemDaemon
{
	...
	/**
	 * We overload pgq\SystemDaemon::main() in order to support our specific
	 * commands too.
	 */
	 ...
	 *
	 * The sleeping is cared about at pgq\SystemDaemon level, in run()
	...
	public function process()
	{
		...
		while (!$sleep && !$this->killed) {
			...
			switch ($batch_id) {
				case null:
					$this->log->verbose("pgq\PGQConsumer.process: next_batch is null, sleep.");
					$sleep = True;
					break;

				case False:
					$this->log->verbose("pgq\PGQConsumer.process: failed to get batch.");
					break;

				case True:
					$this->log->verbose("pgq\PGQConsumer.process: \$this->process_batch(%d).",
						$batch_id);
	...
	public function process_batch($batch_id)
	{
		...
		if ($events === False) {
			$this->log->verbose("pgq\PGQConsumer.preprocess_batch got not events (False).");
			return False;
		}
		...
		foreach ($events as $event) {
			...
			if ($this->preprocess_event($event) === PGQ_ABORT_BATCH) {
				...
				$this->log->verbose("pgq\PGQConsumer.process_batch processed event %d of batch %d, tag %d",
					$event->id, $batch_id, $tag);

				switch ($tag) {
					case PGQ_ABORT_BATCH:
						$this->log->verbose("pgq\PGQConsumer.process_batch got PGQ_ABORT_BATCH");
						$abort_batch = True;
						break;
				...
				if (!$abort_batch) {
					if ($this->postprocess_event($event) === PGQ_ABORT_BATCH) {
						$this->log->verbose("pgq\PGQConsumer.postprocess_event(%d) abort batches", $event->id);
						$abort_batch = True;
					}
				}
			}
	...
	/**
	 * Prepare batch processing, returns pgq\PGQEvent list or False if
	 * empty.
	 *
	 * Exists as a separate function for implementers to accomodate
	 * easily, see pgq\PGQRemoteConsumer.
	 */
	public function preprocess_batch($batch_id)
	{
		...
		if ($events === False) {
			// batch with no event
			$this->log->debug("pgq\PGQConsumer.preprocess_batch got no events");
			$this->finish_batch($batch_id);
			pg_query($this->pg_src_con, "COMMIT;");
			return False;
		}
		$this->log->debug("pgq\PGQConsumer.preprocess_batch got %d events", count($events));
		return $events;
	}
	...
	{
		if ($abort_batch) {
			$this->log->warning("pgq\PGQConsumer.postprocess_batch aborts: ROLLBACK");
			$this->rollback();
			return False;
		}
	...
	public function status()
	{
		$pid = $this->getpid();
		if ($pid === False)
			printf("pgq\PGQConsumer %s is not running.\n", $this->name);
		else {
			printf("pgq\PGQConsumer %s is running with pid %d\n", $this->name, $pid);
	 ...
	 * processing, but don't have the pgq\PGQConsumer daemon quit.
	 */
